...
public class CartServiceImpl implements CartService {
    ...
    public List<Cart> mergeCartList(List<Cart> newCartList,List<Cart> oldCartList){
        ...
        List<OrderItem> newOrderItemList= null;
        if(null!=newCartList&&newCartList.size()>0){
            ...
            if(null!=oldCartList&&oldCartList.size()>0){
                ...
                for (Cart newCart : newCartList) {
                    //1)判断当前新车在老车集合中是否同商家
                    cartIndex = oldCartList.indexOf(newCart);
                    if(cartIndex!=-1){
                        //存在  先获取出 跟新车相同的老车
                        Cart oldCart = oldCartList.get(cartIndex);
                        oldOrderItemList = oldCart.getOrderItemList();
                        newOrderItemList = newCart.getOrderItemList();
